...
##' @param base_alphas Coefficients corresponding to the intercept and X1-X6
##' @param gamma constant determining prevalence of X7
##' @param delta constant determining prevalence of X8
##'
...
##'
##' @author Kazuki Yoshida
generate_sturmer_unmeasured_covariates <- function(df, base_alphas, gamma, delta) {

    assertthat::assert_that(ncol(df) + 1 == length(base_alphas))

    ## Create a design matrix
    Xmat <- cbind(rep(1, nrow(df)), as.matrix(df))

    ## First step P(T = 1 | X1-X6)
    df$pT_first <- locfit::expit(as.numeric(Xmat %*% matrix(base_alphas)))

    ## Unmeasured X7 (prevalence is decreasing in first step PS)
    df$X7 <- as.numeric(runif(n = nrow(df), min = 0, max = 1) <= (gamma - df$pT_first))

    ## Unmeasured X8 (prevalence is increasing in first step PS)
    df$X8 <- as.numeric(runif(n = nrow(df), min = 0, max = 1) <= (df$pT_first - delta))
